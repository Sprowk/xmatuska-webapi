...
func NewMongoService[DocType interface{}](config MongoServiceConfig) DbService[DocType] {
	...
	if svc.DbName == "" {
		svc.DbName = enviro("AMBULANCE_API_MONGODB_DATABASE", "xmatuska-ambulance-wl")
	}
...
func (m *mongoSvc[DocType]) connect(ctx context.Context) (*mongo.Client, error) {
	...
	if len(m.UserName) != 0 {
		...
		uri = fmt.Sprintf("mongodb://%v:%v@%v:%v/?authSource=admin", m.UserName, m.Password, m.ServerHost, m.ServerPort)
	}
